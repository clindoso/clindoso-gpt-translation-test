...
Il numero di workload da configurare e il numero di code da aggiungere a ogni workload dipende solamente dalle esigenze della tua organizzazione. Chiediti se hai bisogno di calcoli del volume, dell’AHT e del fabbisogno di personale separati per ogni coda.

Considera l’esempio seguente.

Il tuo sistema ACD include cinque code: 

- Assistenza tecnica Livello 1
- Assistenza tecnica Livello 2
- Vendite Nord
- Vendite Centro
- Vendite Sud

In questo esempio, le tre code relative alle vendite si riferiscono a diverse regioni di un paese con la stessa lingua principale. I dipendenti qualificati per gestire le richieste relative alle vendite possono quindi farlo indipendentemente dalla regione di origine del contatto. Potrebbe essere utile suddividere le code relative alle vendite in base alla regione per monitorare le campagne di marketing o altri indicatori, ma non è necessario prevedere volumi di contatto o fabbisogni di personale distinti per ciascuna regione. Puoi [creare un workload](#creare-un-workload) con il nome Vendite e selezionare tutte e tre le code relative alle vendite nella sezione **Assegna code**.

Il volume e il fabbisogno di personale per le richieste di supporto, invece, devono essere calcolati separatamente. Le richieste di livello 1 sono più comuni e più facili da gestire rispetto alle richieste di livello 2. I dipendenti qualificati per gestire le richieste di livello 1 non necessariamente sono formati per gestire le richieste di livello 2. Inoltre, poiché le richieste di livello 2 sono meno comuni, avrai bisogno di meno dipendenti per gestirle. In questo caso puoi [creare due workload](#creare-un-workload): Supporto livello 1 e Supporto livello 2, e assegnare a ognuno la coda corrispondente. Se vuoi che i dipendenti responsabili per il livello 2 gestiscano anche le richieste del livello 1 con priorità inferiore, mantieni i due workload separati e utilizza una {% link_new multiattività | features/administration/activity-types-and-properties.md | #subattività %} per la pianificazione.
...
Passa il mouse sopra il grafico per vedere i dettagli sul volume, sul tempo medio di gestione (AHT), sul fabbisogno di personale, sulle modifiche manuali e sugli eventi aggiunti.<br>
Scopri come {% link_new regolare il volume | features/forecast/injixo-forecast/manual-adjustments.md | #regolare-il-volume%}.

### La sezione AHT

La sezione AHT è nascosta per impostazione predefinita quando apri o aggiorni la pagina con i dettagli del workload. Per visualizzare la sezione AHT, clicca sull’{% icon eye_slash %}.
Il grafico con l’AHT è disponibile solo per i workload con code che contengono dati sull’AHT.<br>
Scopri come {% link_new regolare l’AHT | features/forecast/injixo-forecast/manual-adjustments.md | #regolare-laht%}.

### La sezione Fabbisogno di personale

Il grafico nella sezione con il fabbisogno di personale mostra il fabbisogno di personale calcolato.
Sotto il grafico puoi vedere i valori che hai configurato per i parametri e il totale delle ore di lavoro necessarie per il periodo selezionato. Passa il mouse sopra il grafico per vedere i dettagli sul volume, sul tempo medio di gestione (AHT), sul fabbisogno di personale, sulle modifiche manuali e sugli eventi aggiunti.<br>
